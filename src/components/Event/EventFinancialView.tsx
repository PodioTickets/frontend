...
/**
 * Tela Financeiro do evento, compartilhada entre admin e organizer. Canônico =
 * organizer (superset com exportação fiscal, drawers de repasse/estorno/etc.) —
 * por decisão do usuário o admin passa a ter essas features. Header via
 * `renderHeader(event)`; redirect de não-autenticado via `onUnauthenticated`;
 * permissão (`financial`) checada na página do organizer.
 */
export function EventFinancialView({
  eventId,
  onUnauthenticated,
  renderHeader,
}: {
  ...
  const [ticketsPage] = useState(1);
  const [, setTicketsPagination] = useState({
    page: 1,
    limit: 20,
    total: 0,
    totalPages: 1,
  ...
  return (
    <div className="min-h-screen bg-gray-2">
      ...
      <AwaitingReleaseDrawer
        ...
        installmentsTotal={financialData.installmentsToReceive}
