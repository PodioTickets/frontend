...
/**
 * Página "Rastreamento e Conversões" (ads/pixel), compartilhada entre admin e
 * organizer. Antes duplicada byte a byte. O cabeçalho é injetado via
 * `renderHeader(event)` (precisa do `event` carregado aqui dentro); o redirect
 * de não-autenticado vem por `onUnauthenticated`.
 */
export function EventAdsView({
  eventId,
  onUnauthenticated,
  renderHeader,
}: {
