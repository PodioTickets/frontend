...
import { X, XCircle, Building2, MapPin, Phone, ChevronRight, Trash2, Plus } from "lucide-react";
import {
  Drawer,
  DrawerClose,
  DrawerContent,
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-1 text-base leading-[1.3]">
            ...
            <span className="font-medium font-family-dm-sans text-gray-12">{pixKey.pixKey ?? "—"}</span>
          </div>
        </div>
        <ChevronRight
          className={cn(
      ...
      {expanded && (
        <div className="px-5 pb-5 flex flex-col gap-6">
          <div className="flex flex-wrap gap-x-4 gap-y-6">
            <FieldInput label="Tipo de Chave" value={pixKey.pixKeyType ?? "Pix"} readOnly className="min-w-[290px]" />
            <FieldInput label="Chave cadastrada" value={pixKey.pixKey ?? ""} readOnly className="min-w-[290px]" />
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const syntheticPix: PixKey[] = d.pix
          ? [{ id: org.id, bankName: d.bankName, pixKey: d.pix, accountHolderName: d.accountHolderName, accountHolderDocument: formatCPFOrCNPJ(d.accountHolderDocument) }]
          : [];
        setPixKeys(syntheticPix);
      } catch {
        if (cancelled) return;
        setName(org.name ?? "");
        setEmail(org.email ?? "");
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-8 px-5 pt-5 pb-14">
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-6">
                  <FieldInput label="CNPJ" value={formatCNPJ(cnpj)} readOnly className="min-w-[284px]" />
                  ...
                  <FieldInput label="Nome do responsável" value={ownerName} readOnly className="min-w-[284px]" />
                  <FieldInput label="CPF do responsável" value={ownerDocument} readOnly className="min-w-[284px]" />
                </div>
              </div>
