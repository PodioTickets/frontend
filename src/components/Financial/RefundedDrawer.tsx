...
export function RefundedDrawer({
  ...
  const loadRefundedItems = async () => {
    try {
      ...
      toast.error("Erro ao carregar estornados");
      setRefundedItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const formatRefundedForDisplay = (item: RefundedItem) => {
    // refundDate é INSTANTE real → BRT (America/Sao_Paulo).
    const formattedDate = formatDateBRT(item.refundDate, {
      day: "2-digit",
      month: "short",
      year: "numeric",
    });
    const formattedTime = formatTimeBRT(item.refundDate, {
      hour: "2-digit",
      minute: "2-digit",
    });
  ...
  /* Item normalizado pro body mobile — esquema reduzido (avatar, nome, email,
   * valor, paymentMethod, orderId) compartilhado com ChargebackDrawer. */
  const mobileItems: FinancialDetailsItem[] = refundedItems.map((item) => {
    const d = formatRefundedForDisplay(item);
    return {
      id: item.id,
      orderId: d.orderId,
      buyer: d.buyer,
  ...
  const handleMobileItemClick = (mobile: FinancialDetailsItem) => {
    const original = refundedItems.find((i) => i.id === mobile.id);
    if (!original) return;
    const displayItem = formatRefundedForDisplay(original);
    setSelectedPayment({
      ...displayItem,
      ...
      <Drawer open={isOpen} onOpenChange={onClose} direction="right" disablePreventScroll={false}>
        ...
        <DrawerContent data-vaul-no-drag className="bg-gray-1 h-full w-full sm:max-w-[969px] border-l border-gray-6">
          ...
          <FinancialDetailsMobile
            eventName={eventName}
            title="Estornados - Detalhes"
            titleIconBgClass="bg-red-4"
            titleIcon={<RemoveIcon className="size-3 text-red-12" />}
            onClose={onClose}
            totalLabel="Total estornado"
            totalAmount={totalRefunded / 100}
            countLabel={
              <>
                <p className="leading-[1.2] mb-0">Transações</p>
                <p className="leading-[1.2]">estornadas</p>
              </>
            }
                ...
                <div className="flex flex-col items-start w-full">
                  {loading ? (
                    ...
                    refundedItems.map((item) => {
                      const displayItem = formatRefundedForDisplay(item);
                      return (
                        <div
                          key={item.id}
                        ...
                        >
                          ...
                          <div className="flex flex-1 h-full items-center justify-center min-h-px min-w-px p-4">
                            <div className="flex items-center gap-1 justify-center w-full">
                              ...
                              <span className="font-inter font-semibold leading-[1.3] text-sm text-red-11">
