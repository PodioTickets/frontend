...
export default function ReviewQuestionnairePage() {
      ...
      async (payload: QuestionModalLocalPayload | undefined) => {
        if (payload?.kind === "create") {
          ...
          setQuestions((prev) => {
            ...
            const q: Question = { id, question: payload.questionData.question, description: payload.questionData.description, type: payload.questionData.type, options: payload.questionData.options, isRequired: payload.questionData.isRequired ?? true, order, appliesTo: payload.questionData.appliesTo ?? "all", eventId, createdAt: "", updatedAt: "" };
            return [...prev, q].sort((a, b) => a.order - b.order);
          });
          return;
        }
        if (payload?.kind === "update") {
          setQuestions((prev) => prev.map((q) => q.id === payload.questionId ? { ...q, question: payload.questionData.question, description: payload.questionData.description, type: payload.questionData.type, options: payload.questionData.options, isRequired: payload.questionData.isRequired ?? true, appliesTo: payload.questionData.appliesTo ?? "all" } : q));
          return;
        }
  ...
  const isDirty = useMemo(
    () => committedQuestionsJson !== null && JSON.stringify(questions) !== committedQuestionsJson,
    [questions, committedQuestionsJson],
  );

  const discardLocalChanges = useCallback(() => {
    if (committedQuestionsJson == null) return;
    try { setQuestions(JSON.parse(committedQuestionsJson) as Question[]); } catch { toast.error("Não foi possível restaurar o estado anterior."); }
  ...
  const {
    ...
    navigateTarget: `/admin/events/${eventId}/review/topics`,
    onDiscard: discardLocalChanges,
  });

  const handleCreateQuestion = () => openCreateQuestionModal({ eventId: eventId || "mock-event", deferPersistence: true });

  const handleEditQuestion = (question: Question) => {
    if (!eventId) { toast.error("Evento não encontrado"); return; }
    openCreateQuestionModal({ eventId, questionId: question.id, question, deferPersistence: true });
  };

  const handleDeleteQuestion = async (questionId: string) => {
    if (!eventId) { toast.error("Evento não encontrado"); return; }
    if (isPendingQuestionId(questionId)) { setQuestions((prev) => prev.filter((q) => q.id !== questionId)); toast.success("Pergunta removida."); return; }
    try {
  ...
  const handleSaveChanges = async (): Promise<boolean> => {
    ...
    try {
      for (let i = 0; i < questions.length; i++) {
        ...
        await organizerService.createQuestion(eventId, { question: q.question, description: q.description, type: q.type, isRequired: q.isRequired, options: q.options, order: i + 1, appliesTo: q.appliesTo ?? "all" });
      }
      const sorted = await refetchQuestions();
      setCommittedQuestionsJson(JSON.stringify(sorted));
  ...
  return (
    <>
      <WizardStepLayout
        ...
        gutter="5"
        description="Crie perguntas extras para coletar informações dos participantes."
      ...
      >
        <QuestionsGrid
          ...
          onDeleteQuestion={(id) => openDeleteQuestionModal({ onConfirm: () => handleDeleteQuestion(id) })}
        />
      </WizardStepLayout>

      <UnsavedChangesModal open={leavePromptOpen} onClose={dismissLeavePrompt} title="Alterações não salvas" description="Você fez alterações no questionário. Se sair agora, elas serão perdidas." onSaveAndLeave={handleSaveAndLeave} onLeaveWithoutSaving={confirmLeaveWithoutSaving} />
    </>
  );
}
